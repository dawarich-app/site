...
export default function GeoJSONToKML() {
  ...
  const handleConvert = useCallback(async (file) => {
    ...
    try {
      ...
      const kml = toKML(points, { name: file.name.replace(/\.(geojson|json)$/i, '') });

      const blob = new Blob([kml], { type: 'application/vnd.google-earth.kml+xml' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = file.name.replace(/\.(geojson|json)$/i, '.kml');
  ...
  return (
    <Layout title={pageTitle} description={pageDescription}>
      <Head>
        ...
        <meta name="keywords" content="GeoJSON to KML, GeoJSON converter, KML converter, geographic data converter, Google Earth converter" />
      ...
      <div className={styles.container}>
        <div className={styles.contentWrapper}>
          <div className={styles.header}>
            ...
            <p>Convert your GeoJSON files to KML format instantly. All processing happens in your browser.</p>
          </div>
          <div className={styles.mainContent}>
            <div className={styles.converterSection}>
              <FormatConverter fromFormat="GeoJSON" toFormat="KML" onConvert={handleConvert} isConverting={isConverting} />
              {converted && (
            ...
            <div className={styles.infoSection}>
              <div className={styles.infoCard}>
                ...
                <p>GeoJSON is a JSON-based format for encoding geographic data structures, popular in web mapping libraries.</p>
              </div>
              <div className={styles.infoCard}>
                <h2>About KML Format</h2>
                <p>KML (Keyhole Markup Language) is used by Google Earth and Google Maps for displaying geographic data.</p>
              </div>
              <div className={styles.infoCard}>
                <h2>Why Convert GeoJSON to KML?</h2>
                <ul>
                  <li>View your data in Google Earth with rich visualization</li>
                  <li>Share maps with Google Maps users</li>
                  <li>Add custom styling and annotations</li>
          ...
          <div className={styles.ctaPanel}>
            <div className={styles.ctaContent}>
              ...
              <a href="/?utm_source=converter&utm_medium=cta&utm_campaign=geojson-to-kml" className={styles.ctaButton}>Explore Dawarich</a>
            </div>
          </div>
        </div>
      </div>
