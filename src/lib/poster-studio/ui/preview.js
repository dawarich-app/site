// Vendored from dawarich app/javascript/poster_studio/ui/preview.js — edit there first, then re-sync. Intentional deltas: fitFrame filters stage children on data-poster-role instead of Stimulus target attributes (with an extra "chrome" role for overlaid stage chrome); preserveDrawingBuffer false (no blurred-backdrop snapshot on the site).
...
export function createPreviewMap({ container, style, bounds }) {
  const map = new maplibregl.Map({
    ...
    // The site has no blurred-backdrop snapshot, so skip the GPU cost.
...
export function fitFrame(stage, aspect) {
  ...
  const controlsHeight = [...stage.children]
    .filter((el) => {
      const role = el.dataset?.posterRole
      return role !== "frame" && role !== "backdrop" && role !== "chrome"
    })
    .reduce((sum, el) => {
