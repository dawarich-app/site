// Vendored from dawarich app/javascript/poster_studio/ui/order_client.js — edit there first, then re-sync. Intentional deltas: i18n translate() replaced with local English strings; too_large copy reworded for the site (no DPI cap control).
const ERROR_MESSAGES = {
  wrong_size: "The exported PDF doesn't match the selected format — try again.",
  too_large:
    "The exported PDF is too large (50 MB max). Try a smaller print size or zoom out.",
...
export async function submitPrintOrder({
  ...
  if (status < 200 || status >= 300) {
    throw new Error(ERROR_MESSAGES[body.error] || ERROR_MESSAGES.generic)
  }
...
function postForm(url, form, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener("error", () =>
      reject(new Error(ERROR_MESSAGES.connection)),
    )
    xhr.send(form)
  })
}
